...
class Movie():
    ...
    def __init__(self, movie_title, movie_storyline, poster_image, trailer_youtube, movie_rating, movie_type, directors, starring, movie_release_date, movie_length):
        ...
        self.length = movie_length      # they are all instance variables

    def play_movie(self):
        """
        This function will open and play the movie in a new page (tab),
        if possible, on the web browser.
        """
    ...
    def __repr__(self):
        """
        This function is created to list the set of class data members
        available for this Movie class.
        """
        return "<Movie title:%s\n storyline:%s\n poster_image_url:%s\n \
                    trailer_youtube_url:%s\n rating:%s\n type:%s\n \
                    directors:%s\n actors:%s\n release_date:%s>" % \
                    (self.title, self.storyline, self.poster_image_url, \
                     self.trailer_youtube_url, self.rating, self.type, \
                     self.directors, self.actors, self.release_date)